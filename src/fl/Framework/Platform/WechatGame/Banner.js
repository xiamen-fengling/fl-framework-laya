...
var Banner = /** @class */ (function (_super) {
    ...
    // 生命周期(模板方法，以on开头)
    // =======================================
    Banner.prototype.onEnabled = function () {
        var _this = this;
        // 更新banner的自动刷新间隔
        try {
            this.updateBannerInterval = window.serverConfig ? window.serverConfig.LEFT_BANNER_UPDATE_TIME : 3;
            this.loadBannerMaxCount = window.serverConfig ? window.serverConfig.LEFT_BANNER_UPDATE_COUNT : 3;
        }
        catch (error) {
            this.updateBannerInterval = 3;
            this.loadBannerMaxCount = 3;
        }
        this.positionTag === '' && (this.positionTag = this.adConfigKey.trim());
        // 如果清理为0，每次进入该面板都会快速刷新，太浪费了点
        // this.quickUpdateCount = 0;
        Banner.hideAllBannerAd();
        this.scheduleOnce(function () {
            _this.createBanner();
            _this.showOnStart && _this.show();
        }, 0.05);
    };
    Banner.prototype.onDisabled = function () {
        this.hide();
        Banner.hideAllBannerAd();
        FLSystemEvent_1["default"].emit(Banner.EEventName.SHOW_RESIDENT_BANNER_AD, {
    ...
    /** 注册事件 */
    Banner.prototype.onAddEvents = function () {
        ...
        this.resident && FLSystemEvent_1["default"].on(Banner.EEventName.SHOW_RESIDENT_BANNER_AD, this.onShowResidentBannerAd, this);
    };
    /** 取消事件注册 */
    Banner.prototype.onRemoveEvents = function () {
        ...
        FLSystemEvent_1["default"].off(Banner.EEventName.SHOW_RESIDENT_BANNER_AD, this.onShowResidentBannerAd, this);
    };
    /** 初始化配置 */
    // onLoadConfig() {
    // }
    ...
    Banner.prototype.onStart = function () {
    };
    /**
     * 场景动画更新前回调
     * @param dt 游戏帧时长
     */
    Banner.prototype.onUpdate = function () {
        var dt = Laya.timer.delta / 1000;
        if (!this.isAutoUpdate || Banner.loadSuccCount > this.loadBannerMaxCount) {
        ...
        try {
            var cache = Banner.banners[this.adUnitId];
    ...
    // }
    /** 销毁组件 */
    Banner.prototype.onDestroyed = function () {
    };
    // =======================================
    // 引擎事件回调(以on开头)
    // =======================================
    ...
    // }
    // 广告被点击
    Banner.prototype.onTouchBannerAd = function (data) {
        var adUnitId = this.adUnitId;
        var cache = Banner.banners[adUnitId];
        if (!cache || !cache.isShow) {
            return;
        }
        if (Banner.loadSuccCount < this.loadBannerMaxCount) {
            this.destroyBanner();
            try {
                this.createBanner();
                this.show();
    ...
    /** 显示驻留广告 */
    Banner.prototype.onShowResidentBannerAd = function (data) {
    ...
    // }
    /** 初始化广告id */
    Banner.prototype.initBannerId = function () {
        if (this.isTouchByMistake) {
            var ids = [];
            var cache = void 0;
            // 筛选已创建并加载成功的广告id
            for (var key in Banner.banners) {
                cache = Banner.banners[key];
    ...
    /** 创建广告 */
    Banner.prototype.createBanner = function (isShow) {
        ...
        var left = 0;
        var top = Banner.getBannerTop();
        switch (this.styleType) {
            case "LANDSCAPE":
                width = this.width;
                left = Banner.getBannerLeft(this.left);
            ...
            case "LANDSCAPE_CENTER":
                ...
                // left = (fl.getFrameSize().width)-20;
                // top = fl.getVisibleSize().height-20;
                top = this.top * frameSize.height / (index_1["default"].getVisibleSize().height / 2);
                break;
            default:
                break;
        ...
        Banner.banners[adUnitId] = cache;
        ++this.loadCount;
        this.bindADViewEvents(view, cache, adUnitId, isShow);
        if (cache && isShow) {
            cache.view.show();
            // cache.view.style.left = 1 - view.style.realWidth + 20;
            // cache.view.style.top = 1 - view.style.realHeight + 20;
            cache.isShow = true;
            // console.log(view.style.left + "aaaaaaa" + view.style.top)
        }
    };
    /** 绑定广告事件 */
    Banner.prototype.bindADViewEvents = function (view, cache, adUnitId, isShow) {
        var _this = this;
        view.onLoad(function () {
            cache && (cache.isLoaded = true);
            FLSystemEvent_1["default"].emit(Banner.EEventName.LOAD_SUCCESSED_BANNER_AD, {
            ...
            Banner.loadSuccCount += 1;
            cache.view.style.left = 1 - view.style.realWidth;
            cache.view.style.top = 1 - view.style.realHeight;
            // console.log(view.style.left + "aaaaaaa" + view.style.top)
        });
        view.onError(function (err) {
            ...
            if (_this.loadErrorCount > 5) {
                ...
                try {
                    Banner.banners[adUnitId] = undefined;
    ...
    // 验证是否快速刷新
    Banner.prototype.checkQuickUpdate = function (cache) {
    ...
    Banner.prototype.destroyBanner = function () {
        this.unscheduleAllCallbacks();
        var adUnitId = this.adUnitId;
        try {
            var cache = Banner.banners[adUnitId];
    ...
    /** 隐藏所有广告，包括驻留广告 */
    Banner.hideAllBannerAd = function () {
        var cache;
        for (var key in Banner.banners) {
            cache = Banner.banners[key];
            if (!cache || !cache.view || !cache.view.hide) {
                Banner.banners[key] = undefined;
                continue;
            }
            cache.view.hide();
            cache.isShow = false;
    ...
    /** 获取Banner的Left */
    Banner.getBannerLeft = function (left) {
    ...
    // 广告错误
    Banner.prototype.onErrorVideoAd = function (res) {
        var msg;
        switch (res.errCode) {
            case 1000:
                msg = '后端接口调用失败，暂时无法观看';
    ...
    Banner.prototype.sendWarning = function (code) {
        code = code || '-1000';
        var isSend = 0;
